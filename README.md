...
- https://delta.chat/
  Okej, tak tohle mě úplně dostalo. Využívá se standardní STMP/IMAP emailové protokoly. Nad tím je postavená enkrypce. Může se využívat jakýkoliv "chatting server" (bude fungovat i jakýkoliv mejl server). Jsou hezký klienti na všechny OS, můžeš mít propojený zařízení do jednoho profilu. Můžeš mít milijón profilů, na různý věci. Defakto posíláš emaily, akorát to tak nevypadá a jsou šifrovaný. Umí to i hovory a pár dalších vtipných věciček. Máš QR na skamarádění (ale je to jen schovanej link, kterej můžeš poslat). Takže máš E2E enkrypci a vytvářet social graph mohou jenom provideři serverů - a to ještě složitě (jo, je určitě anonymnější briar, ale tohle je zajímavější. a stačí si nahostovat pár (no on stačí jeden) vlastních serverů, a je social graph celkem tajnej,.. celkem)
